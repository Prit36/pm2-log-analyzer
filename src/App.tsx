...
export function App() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      if (paste) {
        setPasteText(paste)
        if (paste.length > 0 && paste.length <= PASTE_PERSIST_LIMIT) pendingPasteRef.current = paste
      }
  ...
  const progressUi = isParsing && progress
    ? {
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
  ...
  async function copyApiForExcel() {
    ...
    const tsv = [h.join('\t'), ...d.map((r) => h.map((k) => String((r as Record<string, string>)[k] ?? '')).join('\t'))].join('\r\n')
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="fixed right-5 top-1/2 z-40 hidden -translate-y-1/2 flex-col gap-2 xl:flex">
        <a href="#api" className="press flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white/90 text-slate-700 shadow-sm backdrop-blur hover:border-indigo-300 hover:text-indigo-700" title="API Latency">
          🚀
        </a>
        <a href="#cron" className="press flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white/90 text-slate-700 shadow-sm backdrop-blur hover:border-emerald-300 hover:text-emerald-700" title="Cron Jobs">
          ⏱️
        </a>
        <button onClick={handleDownloadExcel} className="press flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-md hover:bg-indigo-500" title="Download Excel (2 tabs)">
          ⬇
        </button>
      </div>
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="rise">
              ...
              <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg">
                Surface your slowest APIs and cron jobs from raw PM2 output — built to stay smooth on 50MB+ log files.
              </p>
            </div>
          ...
          <div className="rise rise-d3 flex items-center gap-4">
            ...
            <div className="ticker-mask relative flex-1 overflow-hidden">
              <div className="ticker-track">
                ...
                {tickerItems.length === 0 && <span className="font-mono-data text-[11px] text-slate-400">upload or paste logs to populate the live ticker…</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-7xl space-y-10 px-5 py-10 sm:px-8">
        <section className="rise">
          <FileDrop onFile={handleFile} disabled={!isWorkerReady || isParsing} progress={progressUi} />
          {isParsing && (
            <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
              <span>UI stays interactive while the worker parses in the background.</span>
              <button type="button" className="press rounded border border-rose-200 bg-rose-50 px-2 py-1 font-semibold text-rose-700" onClick={() => cancel()}>
                Cancel
              </button>
            </div>
          )}
          ...
          <div ref={revealKpi} className="reveal lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-3">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-baseline gap-3">
                ...
                {loadedFromFile ? (
                  <span className="rounded-md bg-indigo-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-indigo-700 ring-1 ring-indigo-200">file mode</span>
                ) : pasteText ? (
                  <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700 ring-1 ring-emerald-200">draft saved</span>
                ) : null}
              </div>
              <div className="flex items-center gap-2">
                <button className="press rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 disabled:opacity-40" onClick={handleClearAll} disabled={!hasData && !pasteText}>
                  Clear all
                </button>
                <button
            ...
            {loadedFromFile ? (
              <div className="flex h-72 flex-col items-center justify-center rounded-xl border border-dashed border-indigo-200 bg-indigo-50/40 p-6 text-center">
                <div className="font-display text-lg font-semibold text-slate-900">{sourceName}</div>
                <div className="mt-1 text-sm text-slate-600">{sourceSize != null ? formatBytes(sourceSize) : ''} — parsed off the main thread</div>
                <p className="mt-3 max-w-md text-xs text-slate-500">
                  The raw file is not loaded into the text box (that was freezing the UI). Clear and paste if you want to edit a smaller snippet.
                </p>
              </div>
            ) : (
              <textarea
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              Tip: upload the full <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono-data text-[10px] text-slate-700">api-out.log</code> — parsing stays off the UI thread.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 lg:col-span-2">
            <KpiTile label="Requests parsed" value={formatNum(Math.round(cuRequests))} accent="indigo" sub={`${formatNum(summary.unmatched)} lines skipped`} />
            <KpiTile label="Avg latency" value={formatMs(cuAvg)} accent="sky" sub={`max ${formatMs(cuMax)}`} />
            <KpiTile label="Errors (≥400)" value={formatNum(Math.round(cuErrors))} accent={summary.errors > 0 ? 'rose' : 'slate'} sub={`${formatNum(summary.slow)} calls ≥ 3s`} />
            ...
            <div className="lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Field label="HTTP Status">
                  <select className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium" value={statusFamily} onChange={(e) => setStatusFamily(e.target.value as typeof statusFamily)}>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button className="press rounded border border-slate-200 px-2 py-0.5 text-[10px] font-semibold text-slate-600 hover:bg-slate-50" onClick={selectAllMethods}>
                      All
                    </button>
                    <button className="press rounded border border-slate-200 px-2 py-0.5 text-[10px] font-semibold text-slate-600 hover:bg-slate-50" onClick={clearMethods}>
                      None
                    </button>
                  </div>
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={
                            ...
                            (on ? 'border-indigo-300 bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50')
                          }
                        >
                          {m}
                        </button>
              ...
              <div className="lift overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-gradient-to-r from-white to-slate-50/60 px-5 py-3.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button onClick={() => void copyApiForExcel()} disabled={topRows.length === 0} className="press rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-40">
                      Copy for Excel
                    </button>
                  </div>
                </div>
                <VirtualizedApiTable rows={topRows} height={Math.min(520, 40 + Math.max(topRows.length, 1) * 36)} />
              </div>
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} layout="vertical" margin={{ top: 4, right: 18, bottom: 4, left: 8 }}>
                      ...
                      />
                      <Tooltip cursor={{ fill: 'rgba(99,102,241,0.06)' }} formatter={(v, n) => (n === 'count' ? [formatNum(Number(v)), 'count'] : [formatMs(Number(v)), String(n)])} />
            ...
            <div className="lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2">
              ...
              <div className="mt-4 border-t border-slate-100 pt-4">
                ...
                <div className="mt-2 grid grid-cols-3 gap-1.5">
                  {(
                    ...
                    <button key={k} onClick={() => setCronSortKey(k)} className={'press rounded-md px-2 py-1.5 text-xs font-semibold transition ' + (cronSortKey === k ? 'bg-slate-900 text-white shadow-sm' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              <div className="mt-3">
                <div className="mb-1 flex items-center justify-between text-[10px] font-semibold uppercase tracking-wider text-slate-500">
                  ...
                  <span className="font-mono-data text-slate-700">{formatMs(cronSummary.slowestRun)}</span>
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-gradient-to-r from-white to-emerald-50/40 px-5 py-3.5">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    Duration percentiles from <code className="font-mono-data">[cron] done … ms</code> lines
                  </p>
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={handleDownloadExcel} className="press inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-500">
                  ...
                  <button onClick={() => void copyCronForExcel()} disabled={cronRows.length === 0} className="press rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-40">
                    Copy for Excel
                  </button>
                </div>
              </div>
              <VirtualizedCronTable rows={cronRows} height={Math.min(520, 40 + Math.max(cronRows.length, 1) * 36)} />
            </div>
          </div>
        </section>
        ...
        <section className="lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-sm font-semibold text-slate-900">
              ...
              <span className="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600">{formatNum(unmatchedCount)}</span>
            </h3>
          </div>
          <div className="nice-scroll mt-3 max-h-40 overflow-auto rounded-xl border border-slate-200 bg-slate-50/70 p-3 font-mono-data text-[11px] leading-5 text-slate-700">
            ...
            {unmatchedCount === 0 && <div className="text-slate-400">None — every line parsed.</div>}
            {unmatchedCount > unmatchedSample.length && <div className="mt-2 text-slate-500">…and {formatNum(unmatchedCount - unmatchedSample.length)} more</div>}
          </div>
        </section>
        ...
        <footer className="pb-14 pt-2 text-center">
          ...
          <p className="mt-1 text-[11px] text-slate-400">Runs 100% in your browser · Web Worker parsing · virtualized tables · single-file Excel export</p>
        </footer>
      </main>
    </div>
  )
