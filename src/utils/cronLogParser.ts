...
const CRON_REGEX = /^\s*(?:(?<ts>\d{4}-\d{2}-\d{2}(?:T|\s)\d{2}:\d{2}:\d{2}):\s*)?\[cron\]\s+(?<event>start|done|fail)\s+(?<rest>.+?)\s*$/i
...
export function parseCronLine(line: string): CronEvent | null {
  ...
  if (durMatch && durMatch.groups) {
    name = durMatch.groups.name.trim()
    durationMs = Number(durMatch.groups.duration)
  }

  return {
    raw: line,
    ts: m.groups.ts,
    event: m.groups.event.toLowerCase() as 'start' | 'done' | 'fail',
    name,
    durationMs,
  }
}
...
export function aggregateCron(
  ...
  for (const ev of events) {
    ...
    const bucket =
      map.get(ev.name) ??
      { name: ev.name, starts: 0, durations: [], fails: 0 }
