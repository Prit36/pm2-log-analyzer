...
function MethodBadge({ method }: { method: string }) {
  ...
  return (
    <span
      className={`inline-block shrink-0 rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ring-1 ${cls}`}
    >
      {method}
    </span>
  );
...
function ApiRow({ index, style, rows }: RowComponentProps<ApiRowProps>) {
  ...
  return (
    ...
    >
      ...
      <div className="text-right tabular-nums font-semibold text-amber-700">
        {formatMs(row.maxMs)}
      </div>
      <div
        className={`text-right tabular-nums ${row.errorCount > 0 ? "font-semibold text-rose-600" : "text-slate-400"}`}
      >
...
function CronRow({ index, style, rows }: RowComponentProps<CronRowProps>) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-right tabular-nums ${row.fails > 0 ? "font-semibold text-rose-600" : "text-slate-400"}`}
      >
        {formatNum(row.fails)}
      </div>
...
export function VirtualizedApiTable({
  rows,
  height = 420,
}: {
  rows: AggregatedEndpoint[];
  ...
  if (rows.length === 0) {
    return (
      <div
        className="flex items-center justify-center px-3 py-10 text-sm text-slate-400"
        style={{ height }}
      >
...
export function VirtualizedCronTable({
  rows,
  height = 420,
}: {
  rows: CronAggregated[];
  ...
  if (rows.length === 0) {
    return (
      <div
        ...
        style={{ height }}
      >
        Paste lines containing <code className="font-mono-data">[cron] start|done|fail</code> to
        populate this table.
      </div>
    );
  }
